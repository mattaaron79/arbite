...
> **Update — the docs sweep (post-0.2.0).** This document is a point-in-time design
> record: its history is worth keeping, but it may not read as current. The live
> claims below were therefore **corrected in place** where they named a retired thing
> (the config file, the bucket, the status vocabulary), and this banner records what
> changed and why. Annotating the old names and leaving them standing was the
> alternative; it was rejected because the body is long, and a reader meets the body
> long before they meet a banner.
>
> - **The project config is `.arbite/project.yaml`, not `arbite.yaml`.** The move
>   from the repo-root `arbite.yaml`/`.arbite.yaml` was a hard cut: the old filenames
>   are not consulted at all — no fallback, no deprecation warning, no migration — so
>   a project still holding only one of them behaves as if it had no config. The
>   `sink:`, `agents:` and `review:` keys live in `.arbite/project.yaml`. `references`
>   stores root-relative paths into the arbite dir's `plans/` bucket
>   (`plans/foo.md` == `.arbite/plans/foo.md`), added after `depends_on`.
> - **The non-status bucket is `plans/`, not `planning/`.** Also a hard cut with no
>   alias: an existing `planning/` directory simply remains a non-default bucket.
> - **`review` is a first-class status, not an absence.** Finished work now travels
>   `claim → in_progress → submit → review → accept`, the default path (`review: false`
>   in the config opts out and makes `submit` close the ticket directly), and
>   `arbite reopen` requires `--reason` because it is the reviewer's rejection path.
>
> The live description of the current system is README.md and the generated
> `.arbite/AGENTS.md`; this file remains the rationale for *why* it is shaped that way.
...
- `wishlist/` and `plans/` are **non-status buckets**, not statuses.
  `wishlist/` holds reclassified wishes (see `arbite raw wish` and `arbite
  move ... /wishlist` below) that are parked rather than worked; `plans/` holds
  roadmap notes, spec docs and scratch docs (which are not tickets) and is the
  bucket the `references` field points into. `arbite init` creates both.
...
status: open              # raw | open | in_progress | review | blocked | shelved | closed — mirrors folder
...
references: [plans/spec.md]      # plan docs under the arbite dir's plans/ bucket, root-relative; omitted when empty
...
- `arbite init` — create `.arbite/` folder structure if it doesn't exist
  (status folders plus the non-status `wishlist/` and `plans/` buckets),
...
- `arbite move <id> <folder>` — move a ticket's file to a folder under the
  ...
  `.arbite/wishlist/`, and a longer path like `/plans/ideas` nests
  folders; the destination folder is created if it doesn't exist. Used to
  file tickets into non-status buckets such as `wishlist/` or `plans/`;
...
- `arbite submit <id> [--agent <id>]` — hand finished work off: move it to
  `review/` and set `status: review`, **keeping the assignee** (the author is who a
  reviewer sends the work back to), or close it straight away when the project
  config sets `review: false`
- `arbite accept <id> [--agent <id>]` — the reviewer's close: close a ticket that
  is in `review/`, with the automatic note credited to the accepting agent rather
  than the author; a reviewer who rejects the work uses `arbite reopen --reason`
  instead
- `arbite reopen <id> --reason <text> [--agent <id>]` — move a ticket that is not
  `open` back to `open/` (out of `closed/` or out of `review/`), clear its `closed`
  date and `blocked_by`, update `status`/`updated`, and append an automatic
  "Reopened: <reason>." note (attributed to `--agent`, default `system`); the reason
  is **required** because this is the rejection path out of review, and errors if
...
- `arbite sink [info|init]` — report the active sink (kind, location, capabilities,
  ...
  `ARBITE_SINK`, then a `sink:` key in `.arbite/project.yaml`, then the default (`file`)
...
`ARBITE_SINK`, then `sink:` in `.arbite/project.yaml`, then the default `file`). Both
...
- **The choice is written down, not remembered.** `arbite init` and a successful
  `arbite migrate` write `sink: <kind>` into `.arbite/project.yaml`, so the store a
  command created or copied into is the one later commands read. A per-command flag stays a
