...
- ``bound_at`` -- the matching `StoreBinding.bound_at` for the selected
  workspace, or `None`.
- ``records`` -- a mapping of these thirteen **exact** group keys, always present
  ...
  were empty); the same holds for ``worker_profiles``, ``reservations``,
  ``offers`` and ``packages`` (store-level, exported whatever ``workspace_id``
  is selected):
...
OPTIONAL_RECORD_GROUPS = (
    "lifecycle_intents", "worker_profiles", "reservations", "offers", "packages",
)
...
def export_coordination(sink, *, workspace_id: Optional[str] = None,
    ...
    with store.transaction(write=False) as tx:
        ...
        kept = {
            ...
            # Offers are store-level (they name tickets/reservations): always carried.
            "offers": list(raw["offer"]),
            # Packages are store-level (they name tickets): always carried.
            "packages": list(raw["package"]),
        }
...
def bundle_problems(bundle) -> list:
    ...
    profile_ids |= {o.get("id") for o in group("offers")}
    profile_ids |= {p.get("id") for p in group("packages")}
